...
# Load data
##############################################################################
d = Quandl.get("NASDAQOMX/OMXS30", trim_start="2012-01-02",
               trim_end="2014-01-02")
...
stepSize = np.matrix([[0.0726444646,  0.0009686992, -0.0010722330],
                      [0.0009686992,  0.0003498988, -0.0006930118],
                      [-0.0010722330, -0.0006930118,  0.0023934189]])
stepSize = 0.8 * stepSize

# Run the PMH algorithm
(xhat, thhat) = helpParam.particleMetropolisHastingsSVModel(y, initPar,
                                 nPart, T, helpState.pf_sv, nRuns, stepSize)
...
plt.plot(np.mean(xhat[nBurnIn:nRuns, :], axis=0),
...
plt.subplot(4, 2, 3)
plt.hist(thhat[nBurnIn:nRuns, 0], np.floor(
    np.sqrt(nRuns - nBurnIn)), normed=1, facecolor='#7570B3')
plt.xlabel("mu")
plt.ylabel("posterior density estimate")
plt.axvline(np.mean(thhat[nBurnIn:nRuns, 0]), linewidth=1.5, color='k')

plt.subplot(4, 2, 4)
plt.plot(np.arange(nBurnIn, nRuns, 1),
         thhat[nBurnIn:nRuns, 0], color='#7570B3')
plt.xlabel("iteration")
plt.ylabel("trace of mu")
plt.axhline(np.mean(thhat[nBurnIn:nRuns, 0]), linewidth=1.5, color='k')

plt.subplot(4, 2, 5)
plt.hist(thhat[nBurnIn:nRuns, 1], np.floor(
    np.sqrt(nRuns - nBurnIn)), normed=1, facecolor='#E7298A')
plt.xlabel("phi")
plt.ylabel("posterior density estimate")
...
plt.subplot(4, 2, 6)
plt.plot(np.arange(nBurnIn, nRuns, 1),
         thhat[nBurnIn:nRuns, 1], color='#E7298A')
plt.xlabel("iteration")
plt.ylabel("trace of phi")
plt.axhline(np.mean(thhat[nBurnIn:nRuns, 1]), linewidth=1.5, color='k')

plt.subplot(4, 2, 7)
plt.hist(thhat[nBurnIn:nRuns, 2], np.floor(
    np.sqrt(nRuns - nBurnIn)), normed=1, facecolor='#66A61E')
plt.xlabel("sigmav")
plt.ylabel("posterior density estimate")
...
plt.subplot(4, 2, 8)
plt.plot(np.arange(nBurnIn, nRuns, 1),
         thhat[nBurnIn:nRuns, 2], color='#66A61E')
plt.xlabel("iteration")
