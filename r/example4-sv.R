...
# Load data
##############################################################################
d <- Quandl("NASDAQOMX/OMXS30", start_date="2012-01-02", end_date="2014-01-02", type="zoo")
...
plot(y, col="#1B9E77", lwd=1, type="l", xlab="time", ylab="log-returns", ylim=c(-5,5))
plot(xhat[-1], col="#D95F02", lwd=1.5, type="l", xlab="time", ylab="log-volatility estimate", ylim=c(-2,2) )
...
plot(grid, resTh[1:nPlot,1], col='#7570B3', type="l", xlab="iteration", ylab=expression(mu),ylim=c(-1,1), xlim=c(2500,4000))
abline(h=thhat[1], lwd=1, lty="dotted")

muACF <- acf(resTh[,1], plot=FALSE, lag.max=100)
plot(muACF$lag, muACF$acf, col = '#7570B3', type="l", xlab="iteration", ylab=expression("ACF of "* mu), lwd=2, ylim=c(-0.5,1))
...
plot(grid, resTh[1:nPlot,2], col='#E7298A', type="l", xlab="iteration", ylab=expression(mu),ylim=c(-0.88,1.0), xlim=c(2500,4000))
abline(h=thhat[2], lwd=1, lty="dotted")

phiACF <- acf(resTh[,2], plot=FALSE, lag.max=100)
plot(phiACF$lag, phiACF$acf, col = '#E7298A', type="l", xlab="iteration", ylab=expression("ACF of "* phi), lwd=2, ylim=c(-0.5,1))
abline(h=1.96/sqrt(nIter-nBurnIn), lty="dotted")
abline(h=-1.96/sqrt(nIter-nBurnIn), lty="dotted")
...
plot(grid, resTh[1:nPlot,3], col='#66A61E', type="l", xlab="iteration", ylab=expression(mu),ylim=c(0.0,0.4), xlim=c(2500,4000))
abline(h=thhat[3], lwd=1, lty="dotted")

sigmavACF <- acf(resTh[,3], plot=FALSE, lag.max=100)
plot(sigmavACF$lag, sigmavACF$acf, col = '#66A61E', type="l", xlab="iteration", ylab=expression("ACF of "* sigma[v]), lwd=2, ylim=c(-0.5,1))
abline(h=1.96/sqrt(nIter-nBurnIn), lty="dotted")
abline(h=-1.96/sqrt(nIter-nBurnIn), lty="dotted")
