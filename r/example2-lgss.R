...
res1 <- particleMetropolisHastings(data$y, initialPhi, sigmav, sigmae, noParticles, initialState, noIterations, stepSize = 0.01)
res2 <- particleMetropolisHastings(data$y, initialPhi, sigmav, sigmae, noParticles, initialState, noIterations, stepSize = 0.10)
res3 <- particleMetropolisHastings(data$y, initialPhi, sigmav, sigmae, noParticles, initialState, noIterations, stepSize = 0.50)
...
hist(
  res1[noBurnInIterations:noIterations],
...
abline(v = mean(res1[noBurnInIterations:noIterations]),
       lwd = 1,
       lty = "dotted")

hist(
  res2[noBurnInIterations:noIterations],
  breaks = floor(sqrt(noIterations - noBurnInIterations)),
...
abline(v = mean(res2[noBurnInIterations:noIterations]),
       lwd = 1,
       lty = "dotted")

hist(
  res3[noBurnInIterations:noIterations],
  breaks = floor(sqrt(noIterations - noBurnInIterations)),
...
abline(v = mean(res3[noBurnInIterations:noIterations]),
       lwd = 1,
       lty = "dotted")

# Plot the trace of the Markov chain during 1000 iterations after the burn-in
grid <- seq(noBurnInIterations, noBurnInIterations + 1000, 1)
...
abline(h = mean(res3[grid]),
       lwd = 1,
       lty = "dotted")

# Plot the ACF of the Markov chain
grid <- seq(noBurnInIterations, noIterations, 1)
